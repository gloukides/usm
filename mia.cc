...
void score_array(vector<double> &WEIGHTS, INT n, INT kpower, double * SCORE)//, bool &prec_loss)
{
	...
	{	
		...
		if ( WEIGHTS[i-1] == 0 )
		{
			...
			SCORE[i]=(SCORE[i-1]/WEIGHTS[i-1])*WEIGHTS[i+kpower-1];
			/*double tmp = (SCORE[i-1]/WEIGHTS[i-1])*WEIGHTS[i+kpower-1]; // standard 2-finger computation
			...
			{	
				//cout<<"Possible numerical precision issue. Try larger utility threshold!\n";
				//exit(-1);
				prec_loss=true;
				SCORE[i]=tmp;
			}*/
...
/* The main algorithm: it prints the useful maximal patterns of seq with respect to WEIGHTS and the U threshold -- time complexity is O(n log n) in the worst case */
INT useful_pattern_mining(unsigned char * seq, vector<double> &WEIGHTS, INT n, double U, INT L, char * file)
{
	...
	/* First pass */
	INT maxk = (INT)floor(log2(n));	//the maximum number of iterations for the first loop
	INT k = 0;	
        for ( k = 0; k <= maxk; ++k )             // exponential search over all O(log n) powers of two: 2^0, 2^1, 2^2,..., 2^{log n}
        {

                bool succ = 0;                          // if a power of two is no good for any cluster of occurrences then we will terminate
                ...
                for( INT i = 0; i < n; i ++)            // for all O(n) starting positions we will make a linear (top to bottom) scan of the SA/LCP array
                {
			          ...
			          score += SCORE[SA[i]];           // add to the cumulative score for the current cluster of occurrences
						...
						if(SA[j]+kpower <= n)
                                                {
							PARTIAL_LEN[j] = kpower;           	// store the current power of two that is good for this occurence of the current cluster
			...
			{               
				...
				for (INT j = first; j <= n-1; j++ )           // go through all occurrences of this cluster
				{
					if(SA[j]+kpower<=n)
					{
						PARTIAL_LEN[j] = kpower;           	// store the current power of two that is good
						PARTIAL_SCR[j] = SCORE[SA[j]];      // store the score of this specific occurrence
					}
				}
                        }
		...
		if ( succ == 0 )	break;         	// if kpower is no good for any cluster, terminate because no bigger kpower can be good by monotonicity
		else					// we need to examine the next power of two
		{
			INT nextkpower = 2 * kpower;
			if ( (k+1) <= ( INT ) floor(log2(n)) )	score_array(WEIGHTS, n, nextkpower, SCORE);//, prec_loss);
        	}
					#ifdef PRINT_EXAMPLE
					cout << ":step: " << kpower << endl << "lengths \t";
					for (INT i = 0; i < n; i++){
	...
	INT last_k = min(k,maxk);	//this is the largest power of two that was good for at least one cluster of occurrences
	...
	/* Second pass */
	for (INT k = last_k-1; k >=0 ; --k )		//from 2^{k-1} go backwards to smaller powers of 2 up to 2^0
        {	
                INT first = 0;                          // index of the lex-first element (representative) of a cluster of occurrences with an LCP value of 2^k
                double score = 0;                       // cumulative score (TOTAL sum) for the current cluster of occurrences
		
                INT kpower =1<<k;			  // 2^(k-1)
                
                score_array(WEIGHTS, n, kpower, SCORE);//, prec_loss);

                for( INT i = 0; i < n; i ++)            // for all O(n) starting positions
                {
                	
                        if(i > 0 && kpower < PARTIAL_LEN[i] && LCP[i] >= kpower+PARTIAL_LEN[i] && PARTIAL_LEN[i]==PARTIAL_LEN[i-1])   
                        // check if the power of the current cluster can be extended and if it can be added to the cluster: it needs to have common prefix of length kpower+EXT_LEN[i]           
                        {
			          score += SCORE[SA[i]+PARTIAL_LEN[i]] * PARTIAL_SCR[i];           // add to the cumulative score for the current cluster          
			}
                        else 
                        {                                  	// no such an LCP value and so we need to process the current cluster and start a new one
                        	...
                        	if ( ( compare_float(score, U) || score > U ) && i > 0 )	// check if the score of the current cluster is sufficient
                        	{
                                        for (INT j = first; j < i; j++ )           // go through all occurrences of the current cluster
                                        {
						if(SA[j]+kpower+PARTIAL_LEN[j]<=n)
                                                {	
	                                                PARTIAL_SCR[j] *= SCORE[SA[j] + PARTIAL_LEN[j]];   // update the score of this occurrence of the cluster
                                          		PARTIAL_LEN[j] += kpower;                         // extend the length of this occurrence of the cluster
						}
                                        }
                               }			     

                               if( kpower < PARTIAL_LEN[i] && SA[i]+PARTIAL_LEN[i] <n )     
                               {	                              
                               		score = SCORE[SA[i]+PARTIAL_LEN[i]]*PARTIAL_SCR[i];	// set the new cumulative score to the score of the first element of the new cluster              
					first = i;                                    		// this is the first occurrence of the new cluster                        	
                               }
                               else   // new cluster 
                               {
                        ...
                        if ( i == n - 1 && ( compare_float(score, U) || score > U ) )	//special case: here we process the last cluster seprately & check if the score of the last cluster is sufficient
                        {
				for (INT j = first; j <= n-1; j++ )           // go through all occurrences of the last cluster
				{
					if(SA[j]+kpower+PARTIAL_LEN[j]<=n)
					{							
						PARTIAL_SCR[j] *= SCORE[SA[j]+PARTIAL_LEN[j]];      	// store the score of this specific occurrence of the cluster
						PARTIAL_LEN[j] += kpower;           		// extend the length of this occurence of the cluster
					}
				}
                        }
		}
	...
	/*cout<<"suffixes\n";
		...
		{
			cout<<"SA[i]:"<<SA[i]<<" ";
			for(INT j=SA[i];j<SA[i]+PARTIAL_LEN[i];++j)
				cout<<seq[j];
			cout<<"|"<<PARTIAL_LEN[i]<<", "<<PARTIAL_SCR[i]<<endl;	
		}
	*/
	...
	/*cout<<"Sorted merged w.r.t cluster_id: "<<endl;
...
INT length_longest_useful=0;
	...
	cout<<"Length of longest maximal useful:"<<length_longest_useful<<endl;
        
	//cout<<"Total length of maximal useful:"<<total_length_of_output<<endl;
	std::ofstream out_file;
	string filename = "out_"+string(file); 
